        ...
        <form action="/laporan/filter" method="post">
            ...
            <div class="d-flex">
                ...
                <div class="col-3" style="padding-right: 15px; width: 10rem;">
                    <input type="date" name="tgl_mulai" class="form-control btn-sm btn-outline-secondary" value="{{ old('tgl_mulai') }}">
                    <small><p>Tanggal Mulai</p></small>
                </div>
                <div class="col-3" style="padding-right: 15px; width: 10rem;">
                    <input type="date" name="tgl_selesai" class="form-control btn-sm btn-outline-secondary" value="{{ old('tgl_selesai') }}">
...
<script>
    function cetakLaporan() {
        // Ambil nilai tanggal mulai dan tanggal selesai
        var tgl_mulai = document.querySelector('input[name="tgl_mulai"]').value;
        var tgl_selesai = document.querySelector('input[name="tgl_selesai"]').value;

        // Pastikan tanggal mulai dan tanggal selesai tidak kosong sebelum mengarahkan ke URL cetak
        if (tgl_mulai && tgl_selesai) {
            // Escape nilai tanggal untuk mencegah injeksi skrip
            tgl_mulai = encodeURIComponent(tgl_mulai);
            tgl_selesai = encodeURIComponent(tgl_selesai);

            // Redirect ke URL cetak dengan menyertakan tanggal mulai dan tanggal selesai sebagai query parameters
            window.location.href = "/laporan/filter/cetak?tgl_mulai=" + tgl_mulai + "&tgl_selesai=" + tgl_selesai;
        } else {
